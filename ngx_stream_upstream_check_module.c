/*
 *  2017-12-11 chenziyao(chenziyaos@163.com)
 */
...
#define NGX_STREAM_CHECK_UDP                   0x0004
...
static ngx_int_t ngx_stream_upstream_check_backend_init(
        ngx_stream_upstream_check_peer_t *peer);
...
static ngx_int_t ngx_stream_upstream_check_parse_status_line(
        ...
        ngx_http_status_t *status);
static void ngx_stream_upstream_check_backend_reinit(
        ngx_stream_upstream_check_peer_t *peer);
static ngx_int_t ngx_stream_upstream_check_expect_parse(ngx_stream_upstream_check_peer_t *peer);
static ngx_int_t ngx_stream_upstream_check_parse_match(ngx_stream_upstream_check_ctx_t *ctx, ngx_buf_t *b,
const ngx_str_t *expect);
...
static ngx_check_conf_t *ngx_stream_get_check_type_conf(ngx_str_t *str);
...
static char *ngx_stream_upstream_check_keepalive_requests(ngx_conf_t *cf,
                                                          ngx_command_t *cmd, void *conf);
static char *ngx_stream_upstream_check_backend_send(ngx_conf_t *cf,
                                                 ngx_command_t *cmd, void *conf);
static char *ngx_stream_upstream_check_http_expect_alive(ngx_conf_t *cf,
                                                         ngx_command_t *cmd, void *conf);

static char *ngx_stream_upstream_check_backend_expect(ngx_conf_t *cf,
                                                                                                                  ngx_command_t *cmd, void *conf);
...
ngx_stream_upstream_check_find_shm_peer(ngx_stream_upstream_check_peers_shm_t *peers_shm,
...
static ngx_command_t  ngx_stream_upstream_check_commands[] = {
...

        {
          ...
          NGX_STREAM_UPS_CONF|NGX_CONF_TAKE1,
          ngx_stream_upstream_check_backend_send,
          0,
          0,
          NULL },
        {
          ngx_string("check_backend_expect_alive"),
          NGX_STREAM_UPS_CONF|NGX_CONF_TAKE1,
          ngx_stream_upstream_check_backend_expect,
          0,
          0,
          NULL },
...
/*
* 定义 ngx_stream_module_t结构体变量  ngx_stream_upstream_check_module_ctx，这个结构体主要定义于各个hook函数，context 请求环境
* 与模块相关的上下文，目的在于根据配置，正确设置模块的参数信息。
* 一共可看到6个hook函数，解析不同的配置文件
* ngx_stream_upstream_check_[init | create]_[main|srv|loc]srv_conf 会被nginx调用,保存配置信息
* 初始化一个与模块配置信息相关的结构体
*/
...
/*
*  组合模块，
*  完成nginx模块各个组件的开发，将其组装
*  依次是context、指令数组、模块属性以及事件的回调处理函数，不需要可以置为null，
*/
...
/*
*  类型数组，不同的检测类型对应的处理函数，主要针对于port配置
*/
static ngx_check_conf_t  ngx_check_types[] = {
        ...
        { NGX_STREAM_CHECK_TCP,
                ...
                ngx_null_string,
                0,
                ngx_stream_upstream_check_peer_handler,
                ngx_stream_upstream_check_peek_handler,
                ngx_stream_upstream_check_backend_init,
                ngx_stream_upstream_check_expect_parse,
                ngx_stream_upstream_check_backend_reinit,
                0,
                0 },
        { NGX_STREAM_CHECK_UDP,
                ngx_string("udp"),
                ngx_string("X"),/*
...
ngx_stream_upstream_check_connect_handler(ngx_event_t *event)
{
    ...
    peer->pc.type = (ucscf->check_type_conf->type==NGX_STREAM_CHECK_UDP)?SOCK_DGRAM:SOCK_STREAM;
	...
	//set sock opt "IP_RECVERR" in order to recv icmp error like host unreachable.
    int val = 1;
    ...
    peer->state = NGX_STREAM_CHECK_CONNECT_DONE;

    c->write->handler = peer->send_handler;
    c->read->handler = peer->recv_handler;

    ngx_add_timer(&peer->check_timeout_ev, ucscf->check_timeout);//start connect timeout timer.

    /* The kqueue's loop interface needs it. */
    if (rc == NGX_OK) {
        c->write->handler(c->write); // it is check_type->send_handler.
    }
}
...
ngx_stream_upstream_check_recv_handler(ngx_event_t *event)
{
    ...
    if (peer->state != NGX_STREAM_CHECK_SEND_DONE) {
    ...
    peer->state = NGX_STREAM_CHECK_RECV_DONE;
    ngx_stream_upstream_check_clean_event(peer);
    return;
...
static ngx_int_t
ngx_stream_upstream_check_backend_init(ngx_stream_upstream_check_peer_t *peer)
{
    ngx_stream_upstream_check_ctx_t       *ctx;
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
...
static ngx_int_t
ngx_stream_upstream_check_http_parse(ngx_stream_upstream_check_peer_t *peer)
{
    ngx_int_t                            rc;
...
static ngx_int_t
ngx_stream_upstream_check_expect_parse(ngx_stream_upstream_check_peer_t *peer)
{
    ngx_int_t                            rc;
    ngx_stream_upstream_check_ctx_t       *ctx;
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
    ...
    if ((ctx->recv.last - ctx->recv.pos) > 0) {
        ...
        rc = ngx_stream_upstream_check_parse_match(ctx,
                                                         &ctx->recv,
                                                         &ucscf->expect);
        if (rc == NGX_AGAIN) {
            return rc;
...
static ngx_int_t ngx_stream_upstream_check_parse_match(ngx_stream_upstream_check_ctx_t *ctx, ngx_buf_t *b,
const ngx_str_t *expect){
    u_char ch, *p , first = expect->data[0];
    ngx_int_t rc;
    for (p = b->pos; p < b->last; p++) {
        ...
        if( (rc = ngx_strncasecmp(p, expect->data, expect->len))==0){
            return NGX_OK;
        }
    }
    if( p == b->last){
...
static void
ngx_stream_upstream_check_backend_reinit(ngx_stream_upstream_check_peer_t *peer)
{
    ngx_stream_upstream_check_ctx_t  *ctx;
...
static void
ngx_stream_upstream_check_status_update(ngx_stream_upstream_check_peer_t *peer,
                                        ngx_int_t result)
{
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
...
static void
ngx_stream_upstream_check_clean_event(ngx_stream_upstream_check_peer_t *peer)
{
    ngx_connection_t                    *c;
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
    ngx_check_conf_t                    *cf;
    ...
    peer->state = NGX_STREAM_CHECK_ALL_DONE;
...
ngx_stream_upstream_check(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ucscf->check_type_conf == NGX_CONF_UNSET_PTR) {
        ngx_str_set(&s, "tcp"); //default check type is "tcp".
        ucscf->check_type_conf = ngx_stream_get_check_type_conf(&s);
    }
...
static char *
ngx_stream_upstream_check_backend_send(ngx_conf_t *cf, ngx_command_t *cmd,
                                    void *conf)
{
    ngx_str_t                           *value;
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
...
static char *
ngx_stream_upstream_check_backend_expect(ngx_conf_t *cf, ngx_command_t *cmd,
                                    void *conf)
{
    ngx_str_t                           *value;
    ngx_stream_upstream_check_srv_conf_t  *ucscf;
...
// alloc memory for config struct.
static void *
...
ngx_stream_upstream_check_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    if (ucmcf->peers->peers.nelts > 0) {
        ...
        check_peers_ctx = ucmcf->peers;//init check_peers_ctx obj.
...
ngx_stream_upstream_check_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (!same) { //upstream config has changed.
...
static ngx_stream_upstream_check_peer_shm_t *
ngx_stream_upstream_check_find_shm_peer(ngx_stream_upstream_check_peers_shm_t *p,
                                        ngx_addr_t *addr, ngx_str_t *upstream_name)
{
    ngx_uint_t                          i;
    ngx_stream_upstream_check_peer_shm_t *peer_shm;
...
static ngx_int_t
ngx_stream_upstream_check_init_shm_peer(ngx_stream_upstream_check_peer_shm_t *psh,
...
/////////////////////////checker_status monitor///////////////////////////
//l4checker status begin
//1 cmd define
...
ngx_stream_upstream_check_create_loc_conf(ngx_conf_t *cf)
{
    ...
    uclcf = ngx_pcalloc(cf->pool, sizeof(ngx_stream_upstream_check_loc_conf_t)); //ngx_palloc用于·在nginx内存池中分配一块空间，
    if (uclcf == NULL) {
        return NULL;
    }
...
ngx_stream_upstream_check_status_handler(ngx_http_request_t *r)
{
    ...
    if (peers == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                    "stream upstream check module can not find any check "
